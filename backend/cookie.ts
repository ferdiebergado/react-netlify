...
const baseCookie: Readonly<Omit<Cookie, 'name' | 'value'>> = {
  path: '/',
  secure: true,
  sameSite: 'Lax',
};

export function bakeCookie(name: string, value: string, expiresAt: Date): Cookie {
...
export function clearCookie(cookie: Cookie) {
  cookie.value = '';
  cookie.maxAge = 0;
  return cookie;
}
